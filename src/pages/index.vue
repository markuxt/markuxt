...
const { data: allNews } = await useAsyncData(`home-news-${_locale.value}`, async () => {
  ...
  return mergeByPath(docs, _locale.value, _defaultLocale, _localeOrder.value).slice(0, 3)
}, { watch: [_locale] })
...
const { data: allMembers } = await useAsyncData(`home-members-${_locale.value}`, async () => {
  const members = await queryContent('/members')
    ...
    .where({ _extension: 'md' })
    .find()
  return mergeByPath(members, _locale.value, _defaultLocale, _localeOrder.value)
